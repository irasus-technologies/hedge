...
###### HEDGE Changes starts ######
echo "$(date) Scheduled to restart this container every hour"

while true; do
    sleep 3600

    echo "$(date) Restarting the container based on hourly schedule..."
    # Fetch the pid of security-bootstrapper process and force kill it. [s] in security-bootstrapper ensures the grep command itself is ignored
    pid=`ps -o pid,args | grep [s]ecurity-bootstrapper | awk '{print $1}'`
    kill -9 $pid
