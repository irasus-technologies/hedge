...
if [ "$1" = 'security-bootstrapper' ]; then
  # We want to make sure that configuration.yaml is placed only once. So, if it exists, then do not override it
  if [ -f /edgex-init/bootstrap-redis/res/configuration.yaml ]; then
    echo "/edgex-init/bootstrap-redis/res/configuration.yaml already exists. /edgex-init/bootstrap-redis/res/configuration.tmp.yaml will be removed"
    rm -f /edgex-init/bootstrap-redis/res/configuration.tmp.yaml
  else
    echo "/edgex-init/bootstrap-redis/res/configuration.yaml doesn't exist. /edgex-init/bootstrap-redis/res/configuration.tmp.yaml will be renamed"
    mv /edgex-init/bootstrap-redis/res/configuration.tmp.yaml /edgex-init/bootstrap-redis/res/configuration.yaml
  fi
