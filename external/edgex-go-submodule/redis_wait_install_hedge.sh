...
# Taking additional redis params from /edgex-init/bootstrap-redis/res/configuration.yaml and putting them to the redis config file.
# Those params should be added as a yaml list with name DatabaseConfigAdditionalParams into DatabaseConfig section.
# Example:
# DatabaseConfig:
...
echo "Adding additional params to ${DATABASECONFIG_PATH}/${DATABASECONFIG_NAME}"
awk '
/DatabaseConfigAdditionalParams:/ {
  in_block=1
  next
...
/^[[:space:]]*-[[:space:]]/ && in_block {
  print gensub(/^[[:space:]]*-[[:space:]]/, "", 1)
  next
}
/^\S/ {
  in_block=0
}
' "/edgex-init/bootstrap-redis/res/configuration.yaml" >> "${DATABASECONFIG_PATH}/${DATABASECONFIG_NAME}"
