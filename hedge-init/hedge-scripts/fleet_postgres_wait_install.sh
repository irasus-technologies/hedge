...
###### HEDGE changes starts : added STAGEGATE_DB_HOST for Kubernetes
export STAGEGATE_DB_HOST=0.0.0.0
###### HEDGE changes ends
...
exec su-exec postgres /edgex-init/security-bootstrapper --configDir=/edgex-init/res listenTcp \
  --port="${STAGEGATE_DB_READYPORT}" --host="${STAGEGATE_DB_HOST}"
if [ $? -ne 0 ]; then
  echo "$(date) failed to gating the postgres ready port, exits"
fi
export STAGEGATE_DB_HOST=hedge-fleet-db
